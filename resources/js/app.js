...
Echo.channel('tweets')
    .listen('.TweetLikesWereUpdated', (e) => {
        if (e.user_id === User.id) { //**e** er modhe TweetLikesWereUpdated er id,user_id,count
    ...
    .listen('.TweetRepliesWereUpdated', (e) => {
        store.commit('timeline/SET_REPLIES', e)//unlimited reply j kew tai user check lage ni.line ek user ekbar like korte parbe tai user check korte hobe
